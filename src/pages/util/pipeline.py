...
def get_associate_ABI(xds_path, area):
    """
    Parameters
    ----------
    xds_path : str
    ...
    if 'C07_G16' in xds_path:
        band_id = 7
        associate_folder = os.path.join(area['data_folder'], area['bucket'], 'pred', 'cloud')
        associate_file_lst = os.listdir(associate_folder)
    elif 'C14_G16' in xds_path:
        band_id = 14
        associate_folder = os.path.join(area['data_folder'], area['bucket'], 'pred', 'diff')
        associate_file_lst = os.listdir(associate_folder)
    else:
...
def band_7_process(objectId, area):
    """
    Parameters
    ----------
    objectId : str
    ...
    """
    xds_path = download.download_preproc_ABI(objectId, area, 7)

    if xds_path == None:
        return 

    filepath = predict.predict_xarray(xds_path, area)

    return filepath

def band_14_process(objectId, area):
    """
    Parameters
    ----------
    objectId : str
    ...
    filepath : str
       path to cloud array
    """
    xds_path = download.download_preproc_ABI(objectId, area, 14)

    if xds_path == None:
        return 

    filepath = predict.predict_cloud(xds_path, area)
...
def clear_folders(area):
    ...
    os.path.join(area['data_folder'], 'GLM'),
    ]
...
def initialize_folders(area):
    ...
    """
    folder_lst = [
        os.path.join(area['media_folder'], 'data', area['bucket'], 'actual', 'band_7'),
        os.path.join(area['media_folder'], 'data', area['bucket'], 'actual', 'band_14'),
        os.path.join(area['media_folder'], 'data', area['bucket'], 'pred', 'cloud'),
        os.path.join(area['media_folder'], 'data', area['bucket'], 'pred', 'diff'),
        os.path.join(area['media_folder'], 'data', area['bucket'], 'pred', 'pred'),
        os.path.join(area['media_folder'], 'data', 'GLM'),
        os.path.join(area['media_folder'], 'fires_gifs'),
        os.path.join(area['media_folder'], 'fires_pics'),
        os.path.join(area['media_folder'], 'misc'),
        os.path.join(area['media_folder'], 'tmp')
    ]
    for folder in folder_lst:
...
def callback_ABI(message):
    ...
    # Individual Band processes
        ...
        path = band_7_process(band_path[1], area_info)
        message.ack()

    elif band_path[0] == 14:
        logging.info("Band 14 message recieved")
        path = band_14_process(band_path[1], area_info)
        message.ack()
    ...
    #     pickle.dump([], f)
    bandpath_dct = get_associate_ABI(path, area_info)
    # bandpath_dct = get_associate_ABI(message)

    if bandpath_dct == None:
        return
    else:
        predict.classify(bandpath_dct, area_info)
        return
...
def callback_GLM(message):
    ...
    objectId = message.attributes.get('objectId')
    download.download_GLM_goes16_data(objectId, area_info)
    message.ack()

    # Dealing with cleanup, we don't want any more than the specified number of files in the GLM folder. 
    file_lst = os.listdir(os.path.join(area_info['data_folder'], 'GLM'))
    if len(file_lst) > 25: # this number times 1 minute of data. THIS is how long our search window is
        ...
        try:
            os.remove(os.path.join(area_info['data_folder'], 'GLM', oldest_file))
        except FileNotFoundError:
            pass


def pipeline_california():
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = config.API_KEY
    logging.basicConfig(
        level=logging.INFO,
        filemode='a',
        filename=os.path.join(config.BASE_FOLDER, 'logfiles', 'Cali', f'logfile_{datetime.datetime.now()}.log'),
        datefmt='%H:%M:%S',
    )
    ...
    global area_info 
    area_info = {'name': 'California',
              'bucket': 'ABI_RadC',
              'data_folder': config.NC_DATA_FOLDER_CALI,
              'media_folder': config.MEDIA_FOLDER_CALI,
              'shape': config.CALI_SHP_FOLDER,
              'nn_model': 'band_7_predictor_cali'} 

    subscriber1 = pubsub_v1.SubscriberClient()
    subscriber2 = pubsub_v1.SubscriberClient()
    subscription_path1 = subscriber1.subscription_path(config.GOOGLE_PROJECT_NAME, config.ABI_SUBSCRIPTION_NAME_CALI)
    streaming_pull_future1 = subscriber1.subscribe(subscription_path1, callback=callback_ABI)
    subscription_path2 = subscriber2.subscription_path(config.GOOGLE_PROJECT_NAME, config.GLM_SUBSCRIPTION_NAME_CALI)
    ...
    try: 
        initialize_folders(area_info)
        clear_folders(area_info)
        with open(os.path.join(config.MEDIA_FOLDER_CALI, 'misc', 'classified_lst.pkl'), 'wb') as f:
            pickle.dump([], f)
...
def pipeline_brazil():
    ...
    logging.basicConfig(
        level=logging.INFO,
        filemode='a',
        filename=os.path.join(config.BASE_FOLDER, 'logfiles', 'brazil', f'logfile_{datetime.datetime.now()}.log'),
        datefmt='%H:%M:%S',
    )
    root = logging.getLogger()
    handler = logging.StreamHandler(sys.stdout)
    ...
    global area_info 
    area_info = {'name': 'Brazil',
          'bucket': 'ABI_RadF',
          'data_folder': config.NC_DATA_FOLDER_BR,
          'media_folder': config.MEDIA_FOLDER_BR,
          'shape': config.BR_SHP_FOLDER, 
          'nn_model': 'band_7_predictor_br'}
    ...
    try: 
        initialize_folders(area_info)
        clear_folders(area_info)
        with open(os.path.join(config.MEDIA_FOLDER_BR, 'misc', 'classified_lst.pkl'), 'wb') as f:
            pickle.dump([], f)
        logging.info("Initialized folders and cleared classified list")
    except:
        logging.critical(f"Unable to clear folders\n" + misc_functions.error_handling())
